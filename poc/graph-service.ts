...
export class GraphService {
    ...
    async upsertNorth(userId: string, north: { statement: string; confidence: number }): Promise<string> {
        ...
        `,
                {
                    userId,
                    northId: `north_${userId}_${Date.now()}`,
 ...
 */
export async function testGraphService() {
    ...
    try {
        ...
        const connected = await graph.testConnection();
        ...
        // Create test North
        console.log('\n3. Creating test North...');
        const northId = await graph.upsertNorth('user_test_001', {
            statement: 'Build mobile workout app in 3 months',
            confidence: 85,
        });
        console.log(`✅ Created North: ${northId}`);

        // Add bounds
        console.log('\n4. Adding bounds...');
        await graph.addBound(northId, {
            metric: 'time',
            value: '3 months',
            threshold: 90,
            unit: 'days',
        ...
        console.log('✅ Added 2 bounds');

        // Retrieve North
        console.log('\n5. Retrieving North...');
        const northData = await graph.getNorth(northId);
        console.log('✅ Retrieved:', JSON.stringify(northData, null, 2));
    } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    testGraphService().catch(console.error);
}
